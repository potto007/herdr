...
`tmux list` は、Herdr サーバーが動作しているマシンの既定の tmux サーバーにあるセッションを一覧表示します。各エントリはウィンドウ数、接続中の tmux クライアント数、セッションパス、そして Herdr ワークスペースがすでに接続している場合は `open_workspace_id` を報告します。

`tmux attach` は、ルートペインで指定セッションに対して `tmux attach-session` を実行するワークスペースを作成します。ワークスペースのラベルは既定で `tmux:<session>`、作業ディレクトリは既定でセッションパスです。すでに Herdr ワークスペースを持つセッションに接続すると、そのワークスペースにフォーカスし、2 つ目を作成する代わりに `already_open: true` を返します。ペイン内で tmux からデタッチすると、そのペインには通常のシェルが残ります。Herdr は接続コマンドを永続化するため、セッション復元時に同じ tmux セッションへ再接続します。

セッション名は完全一致が必要で、`:`、`.`、空白を含めることはできません。
