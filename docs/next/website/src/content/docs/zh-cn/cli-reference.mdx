...
`tmux list` 列出运行 Herdr 服务器的机器上默认 tmux 服务器中的会话。每个条目报告窗口数、已连接的 tmux 客户端数、会话路径,以及在已有 Herdr 工作区连接到该会话时的 `open_workspace_id`。

`tmux attach` 创建一个工作区,其根窗格对指定会话运行 `tmux attach-session`。工作区标签默认为 `tmux:<session>`,工作目录默认为会话路径。连接一个已经有 Herdr 工作区的会话时,会聚焦该工作区并返回 `already_open: true`,而不是创建第二个。在窗格内从 tmux 分离后,该窗格保留一个普通 shell。Herdr 会持久化连接命令,因此会话恢复时会重新连接同一个 tmux 会话。
