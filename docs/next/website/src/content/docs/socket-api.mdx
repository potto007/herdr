...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.move`, `agent.start`, `agent.view.set`, `agent.view.clear` |
...
`agent.move` takes `pane_id` and an optional `before_pane_id`, placing that
agent row before the named one, or at the end when `before_pane_id` is omitted.
Both panes must currently appear in the agents panel. The call switches the
panel to the user-defined order, stores the full resulting order, persists it
with the session, and emits `agent.reordered` carrying the new `pane_ids`.

The stored order is server-owned, so every attached client sees it. A client
still chooses which ordering it displays, and one configured for `spaces` or
`priority` sorts locally and ignores this order. Ids that stop resolving, for
example when their pane closes, are skipped rather than stranding the rest.
