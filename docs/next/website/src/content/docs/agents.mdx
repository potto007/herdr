...
Drag an agent row to place it yourself, the same way you drag a space. The panel switches to `user-ordered` and keeps that arrangement, including across a restart. From then on the header label cycles through all three: `grouped`, `priority`, `user-ordered`.
